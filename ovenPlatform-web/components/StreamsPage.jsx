...
import { setupThumbnailTimer, cleanupThumbnail, cleanupAllThumbnails } from "../utils/thumbnailService";
import { setupAllStreamsInfoTimer, cleanupStreamInfoTimer } from "../utils/streamInfoService";
  ...
  useEffect(() => {
    ...
    streams.forEach(stream => {
      if (!thumbnailTimersRef.current[stream.streamName]) {
        const timerId = setupThumbnailTimer(
          stream.streamName, 
          (thumbnailUrl) => updateStreamThumbnail(stream.streamName, thumbnailUrl)
        );
          ...
          <div className="streams-grid">
            {streams.map((stream) => (
              <div 
                ...
                onClick={() => window.location.href = `/player/${stream.streamName}`}
              >
                <div 
                ...
                <div className="stream-info">
                  <div className="stream-header">
                    ...
                    <div className={`viewers-count ${isViewersUpdated[stream.streamName] ? 'updated' : ''}`} title={`${stream.totalConnections || 0} viewers`}>
                      <svg width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
                  ...
                  <div className="stream-status active">Active</div>
                  
                  <div className="stream-detail-item">
                    <span className="detail-label">Start date:</span>
                    <span className="detail-value">{formatDateTime(stream.createdTime)}</span>
                  </div>
...
{/*                   
                  <div className="stream-detail-item">
                    <span className="detail-label">Throughput Out:</span>
                    <span className="detail-value">{((stream.avgThroughputOut || 0) / 1_000_000).toFixed(1)} Mbps</span>
                  </div> */}
                </div>
              </div>
            ))}
