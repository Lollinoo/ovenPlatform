...
class OmeService {
  ...
  async isStreamNameInUse(streamName) {
    try {
      ...
      return activeStreamNames.some(name => 
        name.toLowerCase().trim() === normalizedStreamName
      );
    } catch (error) {
      console.error(`Error checking if stream name "${streamName}" is in use:`, error.message);
  ...
  async getStreamThumbnail(streamName) {
    ...
    try {
      ...
      console.error(`Error fetching thumbnail for stream ${streamName} from ${thumbnailUrl}:`, error.message);
      if (error.response) {
