...
class AuthController {
  ...
  async handleAdmission(req, res) {
    ...
    if (!OME_API_SECRET_KEY) {
      console.error(
        "Auth: OME_API_SECRET_KEY is not configured. Cannot validate X-OME-Signature."
      );
      return res
        .status(500)
        .json({
          error: "Server configuration error: Missing OME API secret key.",
        });
    }
    ...
    try {
      ...
      if (signature !== calculatedSignature) {
        ...
        return res.status(403).json({ error: "Invalid X-OME-Signature" });
      }
      ...
      if (requestStatus === "opening") {
        if (!streamUrl) {
          console.warn(
            "Auth: Stream URL not present in payload for 'opening' status."
          );
          return res
            .status(400)
            .json({ allowed: false, reason: "Missing stream URL" });
        }
        ...
        if (policyParam) {
          try {
            const decodedPolicy = Buffer.from(
              policyParam,
              "base64url"
            ).toString("utf8"); // Use base64url for decoding
            const policyJson = JSON.parse(decodedPolicy);
            if (
              policyJson.url_expire &&
              Number.isFinite(policyJson.url_expire)
            ) {
              policyExpiresTimestampMs = policyJson.url_expire; // Assuming policy.url_expire is already in milliseconds
            } else {
              console.warn(
                `Auth: Invalid or missing 'url_expire' in policy parameter: ${decodedPolicy}`
              );
              return res
                .status(400)
                .json({
                  allowed: false,
                  reason: "Malformed 'policy' parameter: invalid 'url_expire'",
                });
            }
          } catch (e) {
            console.warn(
              `Auth: Error decoding or parsing 'policy' parameter: ${e.message}`
            );
            return res
              .status(400)
              .json({ allowed: false, reason: "Malformed 'policy' parameter" });
          }
        } else {
        ...
        if (!validation.isValid) {
          console.warn(
            `Auth: Invalid stream name "${streamName}": ${validation.error.message}`
          );
        ...
        // Verifica dell'autorizzazione stream tramite servizio Stream
        const authResult = await streamService.isStreamAuthorized(
          streamUrl,
          streamName
        );

        if (authResult.authorized) {
          // Se autorizzato, attiva lo stream nel database
          const sessionId = crypto.randomBytes(16).toString("hex"); // Genera un ID sessione univoco
          await streamService.activateStream(streamName, sessionId);

          console.log(
            `Auth: Stream '${streamName}' authorized via database record. Session: ${sessionId}`
          );
          dynamicallyAuthorizedStreams.add(streamName);
          return res.status(200).json({ allowed: true });
        } else {
          // Se non autorizzato, tenta il fallback con la policy
          console.warn(
            `Auth: Stream database authorization failed: ${authResult.reason}`
          );

          // Fallback: Signed Policy Validation
          if (policySignature && policyExpiresTimestampMs !== undefined) {
            if (!SIGNED_POLICY_SECRET_KEY) {
              console.error(
                "Auth: SIGNED_POLICY_SECRET_KEY is not configured. Cannot validate signed policy."
              );
              return res
                .status(500)
                .json({
                  allowed: false,
                  reason:
                    "Server configuration error: Missing signed policy secret key.",
                });
            }

            console.log(
              `Auth: Found Signed Policy for ${streamName}. Signature: ${policySignature}, Expires (ms): ${policyExpiresTimestampMs}`
            );
            const now = Date.now();

            if (policyExpiresTimestampMs < now) {
              console.warn(
                `Auth: Signed Policy for ${streamName} has expired. Expiration (ms): ${policyExpiresTimestampMs}, Current time (ms): ${now}`
              );
              return res
                .status(403)
                .json({ allowed: false, reason: "Signed Policy expired" });
            }
            ...
            const hmacPolicy = crypto.createHmac(
              "sha1",
              SIGNED_POLICY_SECRET_KEY
            );
            ...
            if (policySignature !== calculatedPolicySignature) {
              console.warn(
                `Auth: Invalid Signed Policy signature for ${streamName}. Received: ${policySignature}, Calculated: ${calculatedPolicySignature}, String Signed: '${stringToSign}'`
              );
              return res
                .status(403)
                .json({
                  allowed: false,
                  reason: "Invalid Signed Policy signature",
                });
            }
            ...
            dynamicallyAuthorizedStreams.add(streamName);
            console.log(
              `Auth: Stream '${streamName}' dynamically authorized (Signed Policy OK). Added to list. Response: { allowed: true }`
            );
            return res.status(200).json({ allowed: true });
          } else {
            console.warn(
              `Auth: Missing Signed Policy parameters for ${streamName}. Request denied.`
            );
            return res
              .status(403)
              .json({
                allowed: false,
                reason: authResult.reason || "Unauthorized streaming attempt",
              });
          }
        }
      } else if (requestStatus === "closing") {
        // Per 'closing' status, aggiorna lo stato nel database e mantieni la logica corrente
        if (streamUrl) {
          const [path] = streamUrl.split("?");
          const urlParts = path.split("/");
          const streamName = urlParts.pop() || urlParts.pop();
          if (streamName) {
            // Disattiva lo stream nel database
            try {
              await streamService.deactivateStream(streamName);
              console.log(
                `Auth: Stream '${streamName}' deactivated in database (closing).`
              );
            } catch (error) {
              console.error(
                `Auth: Error deactivating stream '${streamName}':`,
                error
              );
            }

            // Mantieni la logica di memoria
            dynamicallyAuthorizedStreams.delete(streamName);
            console.log(
              `Auth: Stream '${streamName}' removed from authorized list (closing).`
            );
          }
        }
        ...
        console.log(
          `Auth: Response for status not specified or other than 'opening'/'closing': {}`
        );
